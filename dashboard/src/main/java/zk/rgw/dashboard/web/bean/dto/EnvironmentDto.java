...
@Getter
@Setter
public class EnvironmentDto implements ValidatableDto {

    @NotBlank(message = "环境名称不能为空")
    @Size(min = 3, max = 32, message = "环境名称不能超过32个字符长度，且最少需要3个字符长度")
    @Pattern(regexp = Patters.IDENTIFIER_ZH, message = "环境名称只能包含字母，数字和下划线")
    private String name;
